...
function insertDocuments(db, clientClose, collName, doc) {
    db.collection(collName).insertOne(doc).then(result=>{
...
function updateDocument(db, clientClose, collName, doc1, doc2) {
    db.collection(collName).updateOne(doc1, { $set: doc2 }).then(result=> {
...
function removeDocument(db, clientClose, collName, doc) {
    db.collection(collName).deleteOne(doc).then(result=> {
        assert.equal(1, result.result.n);
...
commander
    ...
    .description('print a list of all collections for [dbs] database; [dbs] defaults to test')
    .action(db=> {
...
commander
    ...
    .action((doc, collName, db)=> {//What if <...> aren't respected?
        clientConnect(db).then(value=> {
            //'{"k":0,"s":1}'
            insertDocuments(value, ()=> client.close(), collName, JSON.parse(doc));
        });
    });
...
commander
    ...
    .action((doc1, doc2, collName, db)=> {
        ...
        clientConnect(db).then(value=> {
            updateDocument(value, ()=> client.close(), collName, JSON.parse(doc1), JSON.parse(doc2));
        });
    });
...
commander
    ...
    .action((doc, collName, db)=> {
        ...
        clientConnect(db).then(value=> {
            removeDocument(value, ()=> client.close(), collName, JSON.parse(doc));
        });
    });
